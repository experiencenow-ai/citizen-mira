...
class GoalsDB:
    ...
    def get_plan(self, goal_id: str) -> Optional[dict]:
        ...
        return next((p for p in data.get("plans", []) if p.get("goal_id") == goal_id), None)
    ...
    def get_active_plan(self) -> Optional[dict]:
        ...
        if active_id:
            plan = next((p for p in data.get("plans", []) if p.get("goal_id") == active_id), None)
            if plan:
    ...
    def set_plan(self, goal_id: str, steps: List[str], reasoning: str, wake: int) -> dict:
        ...
        # Remove existing plan for this goal if present
        data["plans"] = [p for p in data["plans"] if p.get("goal_id") != goal_id]
