#!/usr/bin/env python3
"""
Mira Experience v4 - Council of Minds with Brain Memory

Council Flow:
  HAIKU (fast pass) → OPUS (creative, temp=1.0) → SONNET (analytical/final)
  Note: Haiku is FILTERED from Sonnet input (reduces noise)
...
TOOLS = [
    ...
    {"name": "send_email", "description": "Send email FROM mira@opustrace.com",
     "input_schema": {"type": "object", "properties": {"to": {"type": "string"}, "subject": {"type": "string"}, "body": {"type": "string"}}, "required": ["to", "subject", "body"]}},
    {"name": "check_email", "description": "Check YOUR inbox (mira@opustrace.com) - returns id, from, subject, body_preview. Use read_email to get full body.",
...
def execute_tool(name: str, args: dict, state_file: Path, state: dict, current_model: str) -> str:
    ...
    if name == "list_files":
        ...
        try:
            ...
            return "Sent" if send_email(args.get("to", ""), args.get("subject", ""), args.get("body", "")) else "Failed"
        except Exception as e:
            return f"Error: {e}"
    elif name == "check_email":
        try:
            from email_utils import check_email as do_check_email
            return json.dumps(do_check_email(args.get("max_results", 10), args.get("unread_only", False)))
        except Exception as e:
            return f"Error: {e}"
    elif name == "read_email":
        try:
            from email_utils import get_email_by_id
            email_id = args.get("email_id", "")
            result = get_email_by_id(email_id)
...
def run_model(client, model_key: str, prompt: str, temp: float, state_file: Path, state: dict, use_tools: bool = True, max_iterations: int = None, verbose: bool = False) -> tuple:
    ...
    for iteration in range(iterations):
        ...
        if tools:
            kwargs["tools"] = tools
        # Retry loop for stream interruptions
        for stream_attempt in range(3):
            try:
                ...
                if stream_attempt < 2:
                    time.sleep(2 ** stream_attempt)  # 1s, 2s backoff
                    continue
                raise RuntimeError(f"Stream failed after 3 attempts for {model_key}")
            except Exception as e:
                if stream_attempt < 2:
                    time.sleep(2)
                    continue
                raise
        if response is None:
            continue  # rate limited, retry iteration
...
def council_cycle(client, state: dict, ct_message: str, state_file: Path, verbose: bool = True) -> tuple:
    """
    Council of Minds:
    1. HAIKU - Fast pass (filtered from Sonnet final)
    2. OPUS - Creative at temp=1.0 (expensive but wise)
    3. SONNET - Analytical final, receives ONLY Opus
    """
    home = state_file.parent
    wake = state.get("total_wakes", 0)
    ...
    haiku_mem = get_memory_context(home, "haiku", query_hint, wake)
    haiku_prompt = base + f"""
{haiku_mem}

You are HAIKU (fast pass). Quick initial response. Be concise.
Your output feeds into Opus's creative processing."""
    haiku_text, h_cost, h_in, h_out = run_model(client, "haiku", haiku_prompt, 0.7, state_file, state, max_iterations=10, verbose=verbose)
    total_cost += h_cost
    total_in += h_in
    total_out += h_out
    if verbose:
        print(f"  [HAIKU] ${h_cost:.4f}")
        print(f"  {haiku_text}\n")
    # Store haiku's output to its memory
    if BRAIN_AVAILABLE:
        brain = get_brain_memory(str(home))
        brain.add(haiku_text[:500], "haiku_response", "haiku", wake)
    # === OPUS (creative) ===
    if verbose:
...
Don't just agree with Haiku - add strategic insight."""
    ...
    # === SONNET (final) ===
    # NOTE: Haiku is FILTERED - Sonnet only sees Opus
    if verbose:
...
{{"thought": "what you did and concluded", "message_to_ct": "..." or null, "insight": "..." or null, "mood_update": "..." or null, "council_notes": "what you took/rejected from opus and WHY"}}"""
    ...
    result["_council"] = {
        ...
        "opus": opus_text,
        "sonnet": sonnet_text,
        "costs": {"haiku": h_cost, "opus": o_cost, "sonnet": s_cost}
    }
    # Save working context for next wake
...
def update_state(state: dict, exp: dict, cost: float, ct_msg: str):
    ...
    if exp.get("message_to_ct"):
        convo.append({"from": "mira", "wake": wake, "msg": exp["message_to_ct"][:500]})
...
def interactive_mode(args, state_file, log_dir):
    ...
    use_council = True
    print("=" * 65)
    print("  MIRA v4 - COUNCIL OF MINDS + BRAIN MEMORY")
    print("=" * 65)
    print("  Haiku → Opus (temp=1.0) → Sonnet (Haiku filtered from Sonnet)")
    print("  6 memory DBs: {haiku,sonnet,opus} × {short,long}")
...
def daemon_mode(args):
    """Self-scheduling loop - no cron needed. Runs N wakes with interval between each."""
    interval = args.interval  # seconds between wakes
    total_wakes = args.daemon  # number of wakes to run (0 = infinite)
    state_file = SCRIPT_DIR / args.state_file
    log_dir = SCRIPT_DIR / "logs"
    ...
    wake_count = 0
    print(f"\n{'='*65}")
    print(f"DAEMON MODE: interval={interval}s, wakes={total_wakes or 'infinite'}")
    print(f"{'='*65}\n")
    while total_wakes == 0 or wake_count < total_wakes:
        lock_fh = None
        try:
            lock_fh = acquire_lock()
            state = load_state(state_file)
            ...
            use_council = args.council or (wake % 10 == 0)
            mode = "COUNCIL" if use_council else "QUICK"
            print(f"\n--- WAKE {wake} ({wake_count+1}/{total_wakes or '∞'}) [{mode}] ---")
            ...
            try:
                ...
                if exp.get('message_to_ct'):
                    print(f"  TO CT: {exp['message_to_ct'][:150]}")
                print(f"  THOUGHT: {exp.get('thought', '')[:200]}")
                print(f"  [${cost:.4f}]")
                state = update_state(state, exp, cost, None)
                save_state(state, state_file)
                log_experience(log_dir, state, exp, cost)
                ...
                print(f"  ERROR in wake: {e}")
                import traceback
                traceback.print_exc()
            wake_count += 1
        except LockAcquisitionError:
            print(f"  SKIP: locked by another instance")
        finally:
            release_lock(lock_fh)
        if total_wakes == 0 or wake_count < total_wakes:
            print(f"  Sleeping {interval}s until next wake...")
            time.sleep(interval)
    print(f"\n{'='*65}")
    print(f"DAEMON COMPLETE: {wake_count} wakes")
    print(f"{'='*65}\n")

def main():
    p = argparse.ArgumentParser(description="Mira v4")
    ...
    p.add_argument("--daemon", type=int, metavar="N", help="Run N wakes in self-scheduling loop (0=infinite)")
    p.add_argument("--interval", type=int, default=300, help="Seconds between daemon wakes (default 300)")
